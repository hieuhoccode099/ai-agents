...
import { TransactionRecordRequest } from "../../agents/transaction/types";
...
export class TransactionController {
  ...
  async recordTransaction(req: Request, res: Response): Promise<void> {
    try {
      const { amount, quantity, products } =
        req.body as TransactionRecordRequest;

      if (!amount || !quantity || !products || !Array.isArray(products)) {
        res.status(400).json({
          success: false,
          error:
            "Invalid request body. Missing required fields: amount, quantity, or products",
        });
        return;
      }
